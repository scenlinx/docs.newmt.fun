...
export const sidebar: DefaultTheme.Config['sidebar'] = {
  '/categories/': await getItemsByDate("categories"),
  '/moneys/': await getItems("moneys", []),
  '/others/classics/': await getItems("others/classics", ['山','医','命','相','卜','灵宠','相关经典']),
  '/others/it-serve/': await getItems("others/it-serve", []),
  '/sources/': await getItems("sources", ['每日更新', '影视综艺', '知识付费', '教育学习', '素材资源']),
};

async function getItemsByDate(path: string): Promise<DefaultTheme.SidebarItem[]> {
  let yearGroups: DefaultTheme.SidebarItem[] = [];
  
  const years = await fg(`content/${path}/*`, { onlyDirectories: true, objectMode: true });
  for (const { name: year } of years) {
    let articleItems: DefaultTheme.SidebarItem[] = [];
    const months = await fg(`content/${path}/${year}/*`, { onlyDirectories: true, objectMode: true });
    
    for (const { name: month } of months) {
      const articles = await fg(`content/${path}/${year}/${month}/*`, { onlyFiles: true, objectMode: true });
      
      // 按 Markdown 文件中的 date 字段排序
      articles.sort((a, b) => {
      ...
      for (const article of articles) {
        ...
        articleItems.push({
          text: data.title,
          link: `/${path}/${year}/${month}/${article.name.replace('.md', '')}`,
        });
      }
    }
  ...
  addTopArticles(yearGroups, path);
